...
enum ExportService {
    ...
    static func generateCSV(records: [DailyUsageRecord], deviceName: String) -> URL? {
        ...
        let fileName = "aqualytics-usage-\(sanitize(deviceName)).csv"
    ...
    static func generatePDF(records: [DailyUsageRecord], deviceName: String) -> URL? {
        ...
        let data = renderer.pdfData { context in
            ...
            "Generated by Aqualytics".draw(at: CGPoint(x: margin, y: y), withAttributes: footerAttrs)
        }

        let fileName = "aqualytics-usage-\(sanitize(deviceName)).pdf"
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(fileName)
